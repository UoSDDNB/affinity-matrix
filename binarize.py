...
# load 
celltype_matrix = pandas.read_csv("data/celltype_matrix.csv", index_col = 0)

# clear the old plot
plt.clf()

# draw a histogram of gene expression from the celltype matrix
plt.hist(celltype_matrix.values.flatten(), bins=100)
...
# load
RAW_celltype_matrix = pandas.read_csv("output/RAW_celltype_matrix.csv", index_col = 0)

# draw a histogram of gene expression from the celltype matrix
plt.hist(RAW_celltype_matrix.values.flatten(), bins=5000)
# set the x-axis limits
plt.xlim(0, 30)
# set the y-axis limits
plt.ylim(0, 2000)
...
# binarize the data
celltype_matrix[celltype_matrix > 0] = 1

# check the summary of the matrix
print(celltype_matrix.describe())

# save the matrix as a csv file
celltype_matrix.to_csv("output/celltype_matrix_binary.csv", index = True)